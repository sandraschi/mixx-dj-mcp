...
foreach ($cfg in $configs) {
    ...
    try {
        ...
        if ($json.mcpServers."mixx-dj-mcp" -eq $null) {
            $json.mcpServers | Add-Member -Name "mixx-dj-mcp" -Value @{ url = "http://127.0.0.1:11116/mcp" } -MemberType NoteProperty
            $json | ConvertTo-Json -Depth 10 | Set-Content $cfg.Path
            Write-Host "  Registered mixx-dj-mcp in $($cfg.Name)" -ForegroundColor Green
            $updated++
        } else {
