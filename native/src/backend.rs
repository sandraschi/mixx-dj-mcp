...
const BACKEND_NAME: &str = "mixx-dj-mcp-backend.exe";
const BACKEND_PORT: u16 = 11116;
const BACKEND_TAG: &str = "mixx-dj-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "MIXX_MCP_PORT";
const ENV_HOST: &str = "MIXX_MCP_HOST";
const ENV_TAURI: &str = "MIXX_TAURI";
...
fn free_port(port: u16) -> bool {
    ...
    {
        let img_kill = format!(
            "Stop-Process -Name 'mixx-dj-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'mixx-dj-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM mixx-dj-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM mixx-dj-native.exe /T 2>$null"
        );
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name mixx-dj-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM mixx-dj-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
